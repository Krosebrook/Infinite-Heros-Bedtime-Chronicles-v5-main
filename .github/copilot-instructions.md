...
| Layer | Technology |
|-------|-----------|
| Mobile/Web | Expo SDK 55 + React Native 0.85 + React 19 |
| Routing | Expo Router 6 (file-based, `app/` directory) |
| Backend | Express 5 on Node 20/22 |
| Database | PostgreSQL + Drizzle ORM |
| AI Text | Multi-provider router: Anthropic Claude (primary), Google Gemini, OpenAI, OpenRouter |
| AI Images | Gemini 2.5-flash-image (primary), gpt-image-1 (fallback) |
| TTS | ElevenLabs (9 narrator voices) |
| Video | OpenAI Sora (optional) |
| State | React Context (SettingsContext, ProfileContext) + AsyncStorage |
| Server State | TanStack React Query 5 |
| Animations | react-native-reanimated 4 |
| Types | TypeScript 6.0 strict mode |
...
### General
- TypeScript strict mode — no `any` unless unavoidable; add `// intentional: <reason>` comment.
...
- Rate limiting: per-UID (Firebase) or per-IP via `checkRateLimit()` in `server/rate-limit.ts`.
...
- Child safety rules are in `CHILD_SAFETY_RULES` constant in `server/prompts.ts` — always include in story prompts.
...
constant from `server/prompts.ts`.
...
- `AI_INTEGRATIONS_GEMINI_API_KEY` (primary image/suggestion provider)
...
- Do not use `any` in new code without an `// intentional: <reason>` comment.
- Do not add `@ts-ignore` — fix the type error instead.
- Do not call `AsyncStorage` directly — use helpers in `lib/storage.ts`.
