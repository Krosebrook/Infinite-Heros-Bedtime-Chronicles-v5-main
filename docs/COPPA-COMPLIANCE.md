...
| `@infinity_heroes_parent_controls` | `ParentControls`: max story length, bedtime hour/minute, allowed themes, PIN code, video flag | **Yes — PIN code is a secret** | PIN is stored in plaintext in AsyncStorage. See Section 6 (Gaps). |
...
### GAP 5 — Parent PIN Code Stored in Plaintext (Medium)

**Risk:** Medium. The parental controls PIN is stored in `@infinity_heroes_parent_controls` as a plain string via `AsyncStorage.setItem()`. AsyncStorage on iOS is not encrypted by default; on Android it uses SQLite which may be accessible on rooted devices.

**Recommendation:**
1. Store the PIN as a bcrypt or Argon2 hash (even a short PIN should be hashed).
2. On iOS, use the Keychain via `expo-secure-store` for the PIN value; on Android use the Android Keystore-backed EncryptedSharedPreferences via the same API.
...
| PIN code security | **Stored in plaintext — should be hashed/encrypted** |
